...
describe("Models: Create user", () => {
  ...
  describe('Failure', () => {
    ...
    test("should throw error if email is invalid", async () => {
      ...
      await expect(createUser(user)).rejects.toThrow('email must be a valid email');
    })
    ...
    test("should throw error if username already exists", async () => {
      ...
      await createUser(user);
      await expect(createUser({...user, email: "nayrammensah2@gmail.com" })).rejects.toThrow("username already exists");
    });

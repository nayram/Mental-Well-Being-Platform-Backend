...
describe("API: POST /api/v1/users", () => {
  ...
  describe("Success", () => {
    test("should create user successfully", async () => {
      ...
      const {
        ...
        sql<UserModel.UserSchema>`SELECT * FROM ${sql.identifier([UserModel.UserTableName])} WHERE email = ${user.email} and username = ${user.username}`,
      );
  ...
  describe("Failure", () => {
    test.each([["username"], ["email"], ["password"]])(
      "should fail to create when %s is missing",
      async (key) => {
    ...
    test.each([["username"], ["email"], ["password"]])(
      "should fail to create when %s is not a string",
      async (key) => {

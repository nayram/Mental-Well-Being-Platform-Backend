...
(async () => {
  ...
  await dbClient.connect(async (connection) => {
    return connection.query(sql`INSERT INTO activity (title, description, category, duration, difficulty_level, content, status) VALUES
      ...
      ('Yoga for Beginners', 'Start your yoga journey with some basic poses aimed at improving flexibility and relaxation.', 'PHYSICAL_HEALTH', 1800, 'BEGINNER', 'Follow a series of beginner-friendly yoga poses, focusing on your breath and alignment.', 'ACTIVE'),
      ...
      ('HIIT Workout', 'High-Intensity Interval Training workout to boost your heart rate and improve cardiovascular health.', 'PHYSICAL_HEALTH', 900, 'ADVANCED', 'Perform high-intensity exercises like jumping jacks, burpees, and sprints with short rest intervals.', 'ACTIVE'),
      ...
      ('Rock Climbing Basics', 'Introduction to rock climbing, focusing on technique, safety, and building strength.', 'PHYSICAL_HEALTH', 3600, 'INTERMEDIATE', 'Learn the basics of rock climbing including proper grip, foot placement, and body movement.', 'ACTIVE'),
      ...
      ('Marathon Training Plan', 'A comprehensive training plan for running a marathon, including long runs, speed work, and recovery.', 'PHYSICAL_HEALTH', 7200, 'EXPERT', 'Follow a structured training plan leading up to a marathon, with progressively longer runs.', 'ACTIVE'),
      ('Group Debate', 'Organize a group debate on a chosen topic to improve communication skills and critical thinking.', 'SOCIAL_CONNECTION', 3600, 'INTERMEDIATE', 'Participate in a structured debate, presenting arguments and responding to counterpoints.', 'ACTIVE');`);
  });
  await dbClient.end();
